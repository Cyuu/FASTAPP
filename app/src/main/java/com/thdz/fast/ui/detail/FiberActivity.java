 ...
 */
public class FiberActivity extends BaseActivity implements SurfaceHolder.Callback {
    ...
    public void initView(Bundle savedInstanceState) {
        ...
        String title = "光纤监控";
        try {
            fiberBean = (FiberBean) getIntent().getExtras().getSerializable("bean");
            if (!TextUtils.isEmpty(fiberBean.getAlarmMessage())) {
                title = fiberBean.getAlarmMessage();
            }
            showView();
        } catch (Exception e) {
            toast("无此告警数据~");
    ...
    private void showView() {
        tv_alarm_name.setText(fiberBean.getAlarmMessage());
        tv_alarm_addr.setText(fiberBean.getAreaName());
        tv_alarm_time.setText(fiberBean.getAlarmTime());
        tv_alarm_level.setText(fiberBean.getAlarmLevelName());
        int status = fiberBean.getAlarmStatus();
        String statusVal = "";
        if (status == 0) {
            statusVal = "告警开始";
        } else if (status == 1) {
    ...
    private void requestData() {
        ...
        try {
            obj.put("DeviceId", fiberBean.getDeviceId());
            obj.put("AlarmCode", fiberBean.getAlarmCode());
